...
interface TestSpec { id: string; order: number; test_item: string; specification: string; test_method: string; }
...
export default function FinalProductSpecsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: specsData, error: specsErr } = await supabase.from("labdoc_product_qc_specs").select("*").eq("product_code", decodedProductCode).eq("qc_type", "완제품").order("sequence_no", { ascending: true });
      if (specsErr) { console.error("Final product specs fetch error:", specsErr); setTestSpecs([]); }
      else if (specsData && specsData.length > 0) { setTestSpecs(specsData.filter((s) => s.sequence_no && s.sequence_no > 0).map((s) => ({ id: s.id, order: s.sequence_no ?? 0, test_item: s.test_item || "", specification: s.specification || "", test_method: s.test_method || "" }))); }
      else { setTestSpecs([]); }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white border-2 border-slate-800 print:border-black">
        <div className="text-center py-4 border-b-2 border-slate-800"><h1 className="text-lg font-bold tracking-wider text-slate-800">완제품 시험기준 및 시험방법</h1></div>
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {testSpecs.length === 0 ? <tr><td colSpan={5} className="px-3 py-8 text-center text-slate-400">이 품목의 완제품 시험기준 데이터가 없습니다</td></tr> : testSpecs.map((spec) => (
