    ...
    try {
      ...
      if (productData.semi_product_code) {
        ...
        if (bomData && bomData.length > 0) {
          ...
          const { data: componentsData } = await supabase.from("labdoc_ingredient_components").select("inci_name_en, cas_number, function").in("ingredient_code", baseCodes);
          if (componentsData) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white border border-slate-300 print:border-black text-sm">
        ...
        <Section title="2. PRODUCT APPLICATION"><p>{product.cosmetic_type || "Skin care cosmetics"}</p></Section>
        ...
        <Section title="5. HAZARD IDENTIFICATION"><div className="text-xs"><p className="font-bold">EMERGENCY OVERVIEW :</p><p className="mt-1">This is a personal care or cosmetic product that is safe for consumers and other users under intended and reasonably foreseeable use.</p><p className="font-bold mt-2">POTENTIAL HEALTH EFFECTS :</p><ul className="mt-1 space-y-0.5"><li>● <span className="font-medium">EYE</span> : Exposure may cause mild eye irritation.</li><li>● <span className="font-medium">Skin</span> : May cause irritation or sensitization in sensitive individuals.</li><li>● <span className="font-medium">Inhalation</span> : May cause mild, transient respiratory irritation.</li><li>● <span className="font-medium">Ingestion</span> : Product used as intended is not expected to cause gastrointestinal irritation.</li></ul></div></Section>
        ...
        <Section title="7. FIRE – FIGHTING MEASURES"><div className="text-xs space-y-1"><p><span className="font-bold">Flash Point</span> : Not applicable.</p><p><span className="font-bold">Extinguishing Media</span> : Use chemical foam, dry chemical, carbon dioxide or water.</p><p><span className="font-bold">Explosion Hazard</span> : No applicable information found.</p></div></Section>
        ...
        <Section title="9. HANDLING AND STORAGE"><p className="text-xs">Store in a cool, dry place away from direct sunlight. Keep container tightly closed.</p></Section>
        ...
        <Section title="15. TRANSPORT INFORMATION"><p className="text-xs">Not classified as dangerous goods for transport.</p></Section>
