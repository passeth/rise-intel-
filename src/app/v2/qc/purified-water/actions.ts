...
export async function fetchMeasurements(params: {
  ...
  try {
    let countQuery = fromTable(supabase, 'qc_purified_water_measurements').select('id', {
      count: 'exact',
      head: true,
    })

    let query = fromTable(supabase, 'qc_purified_water_measurements').select('*')
...
export async function createMeasurement(data: {
  ...
  try {
    const { data: measurement, error } = await fromTable(supabase, 'qc_purified_water_measurements')
      .insert(data)
      .select()
      .single()
...
export async function updateMeasurement(
  ...
  try {
    ...
    const { error } = await fromTable(supabase, 'qc_purified_water_measurements')
      .update(updateData)
      .eq('id', id)
...
export async function deleteMeasurement(id: string) {
  ...
  try {
    const { error } = await fromTable(supabase, 'qc_purified_water_measurements').delete().eq('id', id)
...
export async function fetchCertificates(params: {
  ...
  try {
    let countQuery = fromTable(supabase, 'qc_purified_water_certificates').select('id', {
      count: 'exact',
      head: true,
    })

    let query = fromTable(supabase, 'qc_purified_water_certificates').select('*')
...
export async function generateCertificateNo(): Promise<string> {
  ...
  try {
    ...
    const { data, error } = await fromTable(supabase, 'qc_purified_water_certificates')
    ...
    if (data && data.length > 0) {
      ...
      const lastSeq = Number.parseInt(lastNo.split('-')[2], 10)
      sequence = lastSeq + 1
    }
...
export async function createCertificate(data: {
  ...
  try {
    const { data: certificate, error } = await fromTable(supabase, 'qc_purified_water_certificates')
      .insert(data)
      .select()
      .single()
...
export async function updateCertificate(
  ...
  try {
    ...
    const { error } = await fromTable(supabase, 'qc_purified_water_certificates')
      .update(updateData)
      .eq('id', id)
...
export async function deleteCertificate(id: string) {
  ...
  try {
    const { error } = await fromTable(supabase, 'qc_purified_water_certificates').delete().eq('id', id)
...
export async function bulkCreateCertificates(params: {
  ...
  try {
    ...
    if (params.years.length === 1) {
      query = query
        .gte('measurement_date', `${params.years[0]}-01-01`)
        .lte('measurement_date', `${params.years[0]}-12-31`)
    } else {
      const minYear = Math.min(...params.years)
      const maxYear = Math.max(...params.years)
      query = query
        .gte('measurement_date', `${minYear}-01-01`)
        .lte('measurement_date', `${maxYear}-12-31`)
    }

    const { data: measurements, error: fetchError } = await query
    if (fetchError) {
      return { created: 0, skipped: 0, errors: 0, error: fetchError.message }
    }
    ...
    const existingMeasurementIds = new Set(
      (existingCerts || []).map((cert: { measurement_id: string }) => cert.measurement_id)
    )
    ...
    for (const measurement of measurements as WaterMeasurement[]) {
      if (existingMeasurementIds.has(measurement.id)) {
        skipped += 1
        continue
      }

      const measurementYear = new Date(measurement.measurement_date).getFullYear()
      if (!params.years.includes(measurementYear)) {
        continue
      }

      const dateStr = measurement.measurement_date.replace(/-/g, '')
      const certificateNo = `PW-${dateStr}-001`

      const results = params.defaultResults.map((item) => {
        if (item.test_item === 'pH' && measurement.ph_value != null) {
          return { ...item, result: String(Number(measurement.ph_value).toFixed(2)) }
        }
        return { ...item }
      })
      ...
      toInsert.push({
        ...
        test_date: measurement.measurement_date,
        results,
    ...
    for (let index = 0; index < toInsert.length; index += batchSize) {
      ...
      const { error: insertError } = await fromTable(supabase, 'qc_purified_water_certificates').insert(batch)

      if (insertError) {
        console.error(`Batch ${Math.floor(index / batchSize) + 1} error:`, insertError.message)
        errors += batch.length
      } else {
